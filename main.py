...
def process_document(
    ...
    embeddings = get_pages_embeddings(project, "us-central1", pages)
    ...
    with multiprocessing.Pool(len(pages)) as pool:
        ...
        print(f"🔍 {event_id} #1: Succesfully create event pages")
        page_entries = pool.map(process_page, event_pages)
        print(f"🔍 {event_id} #2: Succesfully create page_entries")
        document_entries = list(itertools.chain.from_iterable(page_entries))
        print(f"🔍 {event_id} #3: Succesfully create document_entries")
...
def process_page(event_page: dict) -> list[dict[str, str]]:
    ...
    entries = generate_questions(project, location, text, filename)
    try:
        return [
            {
...
def get_document_text(
    ...
    operation.result()
    print(f"✅ Document AI operation completed: {operation.operation.name}")
    ...
    for blob in storage_client.list_blobs(output_bucket, prefix=output_prefix):
        ...
        for page in document.pages:
            ...
            page_num += 1
            print(f"📝Read results for document on page {page_num}")
            yield "\n".join([document.text[start:end] for (start, end) in segments])
    
    print(f"✅ Document AI Read results completed: {operation.operation.name}")
...
def generate_questions(project: str, location: str, text: str, filename:str) -> list[dict[str, str]]:
    ...
    response = genai_client.models.generate_content(
        ...
        contents=f"Buatlah daftar 20 pertanyaan dan jawaban mandiri yang dapat dijawab dari teks {filename}. Ajukan salah satu pertanyaan untuk merangkum teks tersebut.",
        config=GenerateContentConfig(
            # https://cloud.google.com/vertex-ai/generative-ai/docs/learn/prompts/system-instructions
            system_instruction=[
                "Gunakan bahasa yang sederhana dan kata-kata yang mudah dimengerti.",
                "Hindari istilah teknis dalam jawaban.",
                "Gunakan informasi dalam teks untuk menjawab pertanyaan.",
                text
            ],
